...
export default function AddChallan() {
  ...
  // ── Reset ─────────────────────────────────────────────────────────────────
  const reset = () => {
    setFromProjectId(''); setToProjectId('')
    setChallanNo(''); setDate(today())
    setItems([emptyRow()])
    setNote(''); setEwayBill(''); setTransportMode('')
    setPhotoFile(null); setDocFile(null); setError(null)
  ...
  const handleSave = async () => {
    ...
    const documentUrl = docFile   ? await uploadFile(docFile,   'documents') : null
    ...
    navigate('/dashboard/accounts/invoices')
  }
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-ink-muted text-[13px] mt-0.5">Fill in the details below</p>
        </div>
      </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            <label className={labelCls}>
              From Project <span className="text-danger normal-case">*</span>
            </label>
            <select
              value={fromProjectId}
          ...
          <div className="space-y-2">
            <label className={labelCls}>
              To Project <span className="text-danger normal-case">*</span>
            </label>
            <select
              value={toProjectId}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            <label className={labelCls}>
              Challan No <span className="text-danger normal-case">*</span>
            </label>
            <input
              type="text"
              value={challanNo}
          ...
          <div className="space-y-2">
            <label className={labelCls}>
              Date <span className="text-danger normal-case">*</span>
            </label>
            <input
              type="date"
              value={date}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <span className="truncate">{photoFile ? photoFile.name : 'Choose photo…'}</span>
              </label>
              {photoFile && (
                <button
                  onClick={() => { setPhotoFile(null); if (photoRef.current) photoRef.current.value = '' }}
                  className="p-1.5 text-ink-muted hover:text-danger transition-colors duration-150 shrink-0"
                  title="Remove"
                >
                  <X className="w-3.5 h-3.5" />
                </button>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <Upload className="w-4 h-4 shrink-0" />
                <span className="truncate">{docFile ? docFile.name : 'Choose document…'}</span>
              </label>
              {docFile && (
                <button
                  onClick={() => { setDocFile(null); if (docRef.current) docRef.current.value = '' }}
                  className="p-1.5 text-ink-muted hover:text-danger transition-colors duration-150 shrink-0"
                  title="Remove"
                >
                  <X className="w-3.5 h-3.5" />
        ...
        <div className="flex items-center gap-3 pt-2 flex-wrap">
          ...
          <button
            onClick={reset}
            disabled={saving}
            className="flex items-center gap-2 px-6 py-3 rounded-xl border border-danger/30
              bg-danger/10 hover:bg-danger/20 text-danger font-semibold text-[14px]
