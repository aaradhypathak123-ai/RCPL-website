...
import {
  Wrench, Plus, Pencil, Trash2, Search, X,
...
const EMPTY_FORM: MachineForm = { name: '', type_id: '' }
...
export default function Machines() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [machRes, typeRes] = await Promise.all([
        supabase.from('machines').select('id, name, type_id, machine_types(name)').order('name'),
      ...
      setTypes((typeRes.data ?? []) as MachineType[])
    } catch (e: unknown) {
  ...
  const filtered = useMemo(() => {
    ...
    return machines.filter(m =>
      ...
      (m.machine_types?.name ?? '').toLowerCase().includes(q)
    )
  }, [machines, search])
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (err) throw err
      setAddModal(false); setForm(EMPTY_FORM); await load(); flash('Machine added')
    } catch (e: unknown) {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      if (err) throw err
      setEditTarget(null); setForm(EMPTY_FORM); await load(); flash('Machine updated')
    } catch (e: unknown) {
  ...
  const handleDelete = async () => {
    ...
    setSaving(true)
    try {
      const { error: err } = await supabase.from('machines').delete().eq('id', deleteTarget.id)
      if (err) throw err
      setDeleteTarget(null); await load(); flash('Machine deleted')
    } catch (e: unknown) {
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(
      filtered.map((m, i) => ({ '#': i + 1, 'Machine Name': m.name, 'Type': m.machine_types?.name ?? '—' }))
    )
  ...
  const exportPDF = () => {
    ...
    doc.text('Machines List', 105, 23, { align: 'center' })
    doc.setTextColor(0)
    autoTable(doc, {
      startY: 30,
      head: [['#', 'Machine Name', 'Type']],
      body: filtered.map((m, i) => [i + 1, m.name, m.machine_types?.name ?? '—']),
      theme: 'striped',
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-ink-secondary text-[13px] mt-0.5">
            Manage all construction equipment and vehicles.
          </p>
        </div>
        <button
      ...
      >
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 gap-3">
            <Wrench className="w-8 h-8 text-ink-muted opacity-40" />
            <p className="text-ink-secondary text-[13px] font-medium">
              {search ? 'No machines match your search' : 'No machines yet — add one above'}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full min-w-[480px]">
              <thead>
                <tr className="border-b border-border-subtle">
                  ...
                  <th className="text-left px-5 py-3 text-[11px] font-semibold text-ink-muted uppercase tracking-[0.08em]">Machine Name</th>
                      ...
                      <div className="flex items-center justify-end gap-1">
                        <button
                          onClick={() => {
                            ...
                            setForm({ name: machine.name, type_id: String(machine.type_id) })
                            setFormError(null)
                          }}
      ...
      {/* Add Modal */}
      {addModal && (
        <Modal onClose={() => setAddModal(false)} title="Add Machine">
          <MachineFormFields form={form} setForm={setForm} types={types} />
      ...
      {/* Edit Modal */}
      {editTarget && (
        <Modal onClose={() => setEditTarget(null)} title="Edit Machine">
          <MachineFormFields form={form} setForm={setForm} types={types} />
          {formError && <p className="text-[12px] text-danger">{formError}</p>}
      ...
      {/* Delete Confirm */}
      {deleteTarget && (
        <Modal onClose={() => setDeleteTarget(null)} title="Delete Machine">
...
// ── Form fields ───────────────────────────────────────────────────────────
function MachineFormFields({ form, setForm, types }: {
  form: MachineForm
